...
export function productCardColumns(item: Product) {
  ...
  const fontBaseStyle =
    "w-full min-w-0 text-black text-center text-[12px] sm:text-[14px] md:text-[16px] lg:text-[18px] xl:text-[18px] 2xl:text-[18px] break-words leading-tight";
  const value: Record<string, React.ReactNode> = {
    ...
    ID: (
      <span className={`font-medium ${fontBaseStyle}`}>{item.id_product}</span>
    ),
    ...
    Categoria: (
      <span className={`font-medium ${fontBaseStyle}`}>{item.category}</span>
    ),
    ...
    Marca: <span className={`font-medium ${fontBaseStyle}`}>{item.brand}</span>,

    Ação: (
      <div className="flex flex-col sm:flex-row items-center justify-center gap-1 py-1">
        <div className="flex flex-row gap-1">
          <Tooltip text="Editar produto" position="top">
            <ActionButton
              variant="edit"
              label=""
              icon={Pencil}
          ...
          <Tooltip text="Apagar produto" position="top">
            <ActionButton
              variant="delete"
              label=""
              icon={Trash2}
