...
- [11. Status do Projeto](#-status-do-projeto)
    - [11.1. Em desenvolvimento ➜ Construção do MVP (v1.5.0)](#-em-desenvolvimento--️-construção-do-mvp-v150)
...
#### 🟡 Em desenvolvimento ➜ 🏷️ Construção do MVP (v1.5.0)

Com a arquitetura base e o Design System (*Mobile-First*) plenamente estabelecidos, o projeto estruturou todo o seu ecossistema de navegação global, contando com componentes de *Sidebar*, *Footer* responsivo e um *Header* que realiza o redirecionamento dinâmico para a Home a partir do clique no logo. Atualmente, a aplicação possui **3 telas totalmente funcionais** integradas ao **módulo de produtos**. Isso inclui a listagem dinâmica em tabela responsiva, barra de pesquisa, modais reutilizáveis, e uma tela de cadastro modular com *tooltips* informativos, persistência de dados e um sistema completo de validações de *frontend* com *feedback* em tempo real. Além disso, a aplicação conta com um **sistema avançado de filtros e ordenação** multi-categoria (por ordem alfabética, faixa de preço, níveis de estoque e proximidade de vencimento) e com o **fluxo completo de CRUD** (criação, leitura, edição e exclusão de produtos), integrados de forma dinâmica a uma API simulada (`json-server`). Na tela Home, a implementação de **indicadores-chave** (como custo total de itens vencidos) e o **log interativo de atividades recentes** rastreando as movimentações de produtos consolidam a entrega desta etapa e formam a base estrutural para o futuro *Dashboard*.

No momento, com as funcionalidades essenciais do *frontend* concluídas, o foco da equipe está no **refinamento final e polimento** da aplicação para o fechamento do MVP. As tarefas atuais envolvem um grande esforço da equipe em melhorias contínuas de **acessibilidade e estruturação semântica** (revisão de HTML, implementação de atributos `aria-label` e aprimoramento da ordem de navegação por teclado/TAB em todos os componentes), além de **otimizações de UI/UX** (alinhamento refinado da barra de pesquisa, padronização global de *tooltips* no *Design System* e balanceamento de *helper texts*). Em paralelo, o desenvolvimento foca em validações adicionais de regra de negócio (como o bloqueio de datas de validade retroativas) e na finalização de fluxos de segurança (como o modal de confirmação de exclusão). Esse esforço contínuo visa consolidar definitivamente a interface e a usabilidade do MVP, preparando o terreno para a futura integração com o *Backend* definitivo.
